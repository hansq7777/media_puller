...
class DownloaderGUI:
    ...
    def _build_widgets(self) -> None:
        ...
        tk.Checkbutton(self.root, text="记录已下载", variable=self.archive_var).grid(row=2, column=0, sticky="w")

        # Rate limit
        tk.Label(self.root, text="速率限制").grid(row=3, column=0, sticky="e")
        ...
        self.rate_combo.grid(row=3, column=1, padx=5, pady=5, sticky="w")

        # Dedupe checkbox
        self.dedupe_var = tk.BooleanVar()
        tk.Checkbutton(self.root, text="下载后查重", variable=self.dedupe_var).grid(row=4, column=0, sticky="w")

        # Start button
        tk.Button(self.root, text="开始下载", command=self.start_download).grid(row=5, column=0, columnspan=2, pady=10)
    ...
    def start_download(self) -> None:
        ...
        archive_path = "downloaded.txt" if self.archive_var.get() else None
        rate = self.rate_var.get() or None

        command = gallery_dl_wrapper.build_command(
            url,
            self.cookies_path,
            archive_path,
            rate,
        ...
        thread = threading.Thread(target=self._run_download, args=(command,))
        thread.start()

    def _run_download(self, command) -> None:
        """Execute gallery-dl and handle completion or failure."""
        try:
            gallery_dl_wrapper.run(command)
